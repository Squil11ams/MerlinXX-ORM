...
namespace MerlinORM.Client
{
    ...
    /// </summary>
    public abstract class RelationalQueryEngine
    {
        #region FIELDS
        /// <summary>The resolved provider connection string.</summary>
        ...
        protected RelationalQueryEngine(string ConnectionStringKey, string AppSettings = "appsettings.json")
        {
            ConnectionString = MerlinConfig.GetConnectionString(ConnectionStringKey, AppSettings);
        ...
        /// <summary>
        /// Builds a query engine using an application's existing .NET configuration pipeline.
        /// </summary>
        /// <param name="configuration">The application configuration containing connection strings.</param>
        /// <param name="connectionStringKey">The key within the ConnectionStrings section.</param>
        protected RelationalQueryEngine(IConfiguration configuration, string connectionStringKey)
        {
            ConnectionString = MerlinConfig.GetConnectionString(configuration, connectionStringKey);
        ...
        #region UTILITIES
        /// <summary>Creates and opens a provider connection.</summary>
        protected abstract DbConnection CreateConnection();

        /// <summary>Creates and asynchronously opens a provider connection.</summary>
        protected abstract Task<DbConnection> CreateConnectionAsync(CancellationToken cancellationToken = default);

        /// <summary>Creates a provider command for the supplied Merlin query.</summary>
        protected abstract DbCommand CreateCommand(IMerlinProvider provider, DbConnection connection, bool autoParams = true);
        ...
        private async Task<T> ExecuteAsync<T>(Func<Task<T>> action, string errorCode = "MERLIN-QEP-10001")
        {
            try
            {
                return await action();
